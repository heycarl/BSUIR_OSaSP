...
const char PROJECT[] = "";
const char GENFILES_PATH[] = "/home/sasha/CLionProjects/OSaSP_LABS/cmake-build-debug/";
...
void mergePhase(args* arguments) {
    ...
    printf(YELLOW(YELLOW("thread %lu") "") ": merging succeed\n", arguments -> no);
}
...
static void* execute(args* arguments) {
    printf(YELLOW("thread: %lu") ": start\n", arguments->no);
    while (FILESIZE > curSize) {
        pthread_barrier_wait(&barrier);
        printf(YELLOW("thread: %lu") ": start\n", arguments->no);
        sortPhase(arguments);
        printf(YELLOW("thread: %lu") ": w8ting\n", arguments->no);
        pthread_barrier_wait(&barrier);
